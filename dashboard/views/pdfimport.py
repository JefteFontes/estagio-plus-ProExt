...
def importar_pdf(request):
    ...
    if request.method == 'POST' and request.FILES.get('pdf_file'):
        ...
        try:
            ...
            estagiario = Estagiario.objects.filter(email=estagiario_data.get('email', '')).first()
            if not estagiario:
            ...
            if not all([estagio.bolsa_estagio, estagio.area, estagio.descricao]):
                estagio_incompleto_id = estagio.id  # Identifica o registro para complementação
        ...
        if estagio_incompleto_id:
            return redirect(reverse('complementar_estagio', args=[estagio_incompleto_id]))

    return render(request, 'dashboard_instituicao.html', {
