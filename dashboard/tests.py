...
from .models import (
    Endereco, Instituicao, CoordenadorExtensao, Empresa,
    Cursos, Estagiario, Supervisor, Estagio
)
...
class ViewsIntegrationTestCase(TestCase):
    def setUp(self):
        ...
        self.endereco = Endereco.objects.create(
            rua="Rua Exemplo", numero="123", bairro="Centro",
            cidade="Cidade X", estado="Estado Y", cep="12345678"
        )

        self.instituicao = Instituicao.objects.create(
            cnpj="12345678901234", nome="Instituição X",
            email="instituicao@example.com", telefone="11987654321",
            endereco=self.endereco
        )

        self.user = User.objects.create_user(username="coordenador", password="senha123")
        ...
        self.empresa = Empresa.objects.create(
            instituicao=self.instituicao, empresa_nome="Empresa Teste",
            cnpj="98765432109876", razao_social="Empresa Teste Ltda",
            email="empresa@example.com", atividades="Atividades de teste",
            endereco=self.endereco
        )

        self.curso = Cursos.objects.create(
            instituicao=self.instituicao, nome_curso="Ciência da Computação",
            descricao="Curso de TI", area="Tecnologia",
            coordenador="Maria", email_coordenador="maria@example.com"
        )
...
class IntegracaoModelsTestCase(TestCase):
    def setUp(self):
        ...
        self.estagio = Estagio.objects.create(
            bolsa_estagio=1200.00, area="TI",
            tipo_estagio="Obrigatório", status="Em andamento",
            descricao="Estágio na área de TI", data_inicio="2024-01-10",
            data_fim="2024-07-10", turno="Manha",
            auxilio_transporte=300.00, estagiario=self.estagiario,
            empresa=self.empresa, supervisor=self.supervisor,
            instituicao=self.instituicao, orientador="Professor X"
        )
